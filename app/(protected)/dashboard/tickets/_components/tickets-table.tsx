...
export function TicketsTable({ data }: TicketsTableProps) {
  ...
  const [selectedTicket, setSelectedTicket] = React.useState<TicketListItem | null>(null)
  ...
  const handleRowClick = (ticket: TicketListItem, event: React.MouseEvent) => {
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {ticket.ticket_service_tags && ticket.ticket_service_tags.length > 0 ? (
                          ticket.ticket_service_tags.slice(0, 2).map((serviceTag: ServiceTag) => (
                            <Badge key={serviceTag.id} variant="outline" className="text-xs">
                              {serviceTag.tag}
                            </Badge>
                          ))
                        ) : (
                          <span className="text-xs text-muted-foreground italic">Sin número de series</span>
                        )}
                        {ticket.ticket_service_tags && ticket.ticket_service_tags.length > 2 && (
                          <Badge variant="secondary" className="text-xs">
                            +{ticket.ticket_service_tags.length - 2}
                          </Badge>
                        )}
                      </div>
                    </div>
