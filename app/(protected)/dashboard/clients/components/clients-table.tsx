...
export function ClientsTable({ data }: ClientsTableProps) {
  ...
  const [selectedClient, setSelectedClient] = React.useState<ClientWithStats | null>(null)
      ...
      <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
        <div className="flex flex-1 items-center space-x-2">
          <Input
            ...
            value={(table.getState().columnFilters.find(f => f.id === "company_name")?.value as string) ?? ""}
            onChange={(event) => {
              const value = event.target.value;
              if (value) {
                // Aplicar filtro a las columnas company_name y email
                table.setColumnFilters([
                  { id: "company_name", value },
                  { id: "email", value }
                ]);
              } else {
                // Limpiar filtros cuando el campo está vacío
                table.resetColumnFilters();
              }
            }}
            className="h-8 w-full md:w-[300px] lg:w-[400px]"
          />
          {isFiltered && (
            <Button
              variant="ghost"
