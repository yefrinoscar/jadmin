-- Migration: Public Ticket Submission API
-- Date: 2024-01-16
-- Description: 
...
-- ==================================================
-- STEP 1: Create function to handle public ticket submission
-- ==================================================
...
BEGIN
    ...
    INSERT INTO public.tickets (
        ...
        p_photo_url,
        NULL  -- NULL for public submissions - will be assigned to approving admin later
    );
...
-- ==================================================
-- STEP 2: Modify tickets table to allow NULL reported_by for public submissions
-- ==================================================

-- Allow NULL in reported_by for public submissions
-- We'll identify public submissions by reported_by = NULL and status = pending_approval
...
BEGIN
    ...
    ELSE
        -- Reject the ticket and assign the admin as reported_by if it's a public submission
        UPDATE public.tickets
        SET 
            status = 'closed',
            approved_by = p_admin_user_id,
...
-- ==================================================
-- STEP 6: Update RLS policies to allow anonymous public submissions
-- ==================================================
...
-- Allow anonymous users to insert into clients table for public submissions
CREATE POLICY "Anonymous can create clients for public submissions" ON public.clients
    FOR INSERT WITH CHECK (
        auth.uid() IS NULL AND address = 'Pending verification - submitted via public form'
    );

-- Allow anonymous users to insert into service_tags table for public submissions
CREATE POLICY "Anonymous can create service tags for public submissions" ON public.service_tags
    FOR INSERT WITH CHECK (
        auth.uid() IS NULL AND description LIKE '%requires admin verification%'
    );

-- Allow anonymous users to insert into ticket_service_tags for public submissions
CREATE POLICY "Anonymous can link service tags for public submissions" ON public.ticket_service_tags
    FOR INSERT WITH CHECK (
        auth.uid() IS NULL
    );
...
BEGIN
    ...
    RAISE NOTICE 'Modified tickets table to allow NULL reported_by for public submissions';
    RAISE NOTICE 'Updated RLS policies to allow anonymous public submissions';
