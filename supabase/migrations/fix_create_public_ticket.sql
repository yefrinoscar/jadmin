-- Modified function with explicit type casts to fix the "operator does not exist: text = uuid" error
...
BEGIN
    ...
    IF v_client_record IS NULL THEN
        -- Add explicit cast to UUID when comparing
        v_client_record := (SELECT * FROM public.clients WHERE id = v_provisional_client_id::UUID);
    ...
    INSERT INTO public.tickets (
        ...
        v_client_record.id::UUID, -- Add explicit cast to UUID
    ...
    LOOP
        ...
        AND client_id = v_client_record.id::UUID; -- Add explicit cast to UUID
        ...
        VALUES (v_new_ticket_id::TEXT, v_service_tag_record.id::TEXT); -- Add explicit casts to TEXT
    END LOOP;
